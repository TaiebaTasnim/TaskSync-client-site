...
// }, [tasks]);
 ...
 const onDragEnd = async (result) => {
      ...
      // If dropped within the same category, reorder tasks
      if (destination.droppableId === source.droppableId) {
        ...
        await reorderTasksInDatabase(reorderedTasks);
      } else {
        // If dropped in a different category, update the category
        ...
        await updateTaskInDatabase(draggableId, {
          category: destination.droppableId,
        });
      }
  
      refetch(); // Refetch to get updated tasks from the server
    };
                      ...
                      <Draggable key={task._id} draggableId={task._id} index={index}>
                        {(provided) => (
                          <div
                            ...
                            className="p-4 bg-white text-black rounded-lg shadow-lg flex flex-col justify-between h-80"
                          >
                            ...
                            <p className="flex items-center gap-2 flex-grow"><FaClock></FaClock>{new Date(task.timestamp).toLocaleDateString("en-US",options)}</p>
...
//     <div className=" gap-4 p-4 bg-white min-h-screen text-white grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 ">
//       {categories.map((category) => (
//         <Droppable key={category.key} droppableId={category.key} className="p-4 bg-[#F5556D] rounded-lg">
//           <h2 className="text-xl font-bold mb-4">{category.title}</h2>
//           <div className="grid  gap-4">
//             {tasks
//               .filter((task) => task.category === category.key)
